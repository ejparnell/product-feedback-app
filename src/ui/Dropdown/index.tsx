    ...
    return (
        <div className={styles.dropdown}>
            ...
            <div className={styles.dropwdown__selector} onClick={() => setIsOpen(prev => !prev)}>
                <p className={styles.dropdown__label}><span className={styles['dropdown__text--name']}>Sort by: </span>{selected.label}</p>
                {
                    isOpen ?
                    ...
                    {options.map(option => (
                        <div key={option.value} onClick={() => setSelected(option)}>
                            <div className={styles.dropdown__option}>
                                <p className={`${option.value === selected.value ? styles['dropdown__text--selected'] : styles.dropdown__text}`}>{option.label}</p>
                                {option.value === selected.value && <img className={styles.dropdown__check} src='/assets/shared/icon-check.svg' alt='Check' />}
                            </div>
                            {option.value !== options[options.length - 1].value && <hr className={styles.dropdown__hr} />}
                        </div>
                    ))}
                </div>
            )}
